...
namespace Workshop_Tokeniser
{
    ...
    static Token parse_op(TokenKind kind,string spelling)
    {
        // always read the next character - we have read past the end of the token
        nextch() ;
    ...
    static Token parse_relop(TokenKind kind,string spelling)
    {
        // always read the next character, ch is next char
        nextch() ;
    ...
    static Token parse_keyword(TokenKind kind,string spelling)
    {
        // always read the next character
        nextch() ;
    ...
    Token next_token()
    {
        ...
        while ( ch != EOF )	            // loop until EOF or the start of a token is found
        {
            ...
            case '=':       return parse_single_char_symbol(tk_assign,spelling) ;
            ...
            case '"':       return parse_single_char_symbol(tk_dquote,spelling) ;
                                        ...
                                        // ...
            case '+':       return parse_op(tk_add, spelling);
            case '-':       return parse_op(tk_sub, spelling);
            case '*':       return parse_op(tk_times, spelling);
            case '/':       return parse_op(tk_divide, spelling);
            ...
            case '<':        return parse_relop(tk_lt, spelling);
            case '<=':       return parse_relop(tk_le, spelling);
            case '==':       return parse_relop(tk_eq, spelling);
            case '!=':       return parse_relop(tk_ne, spelling);
            case '>=':       return parse_relop(tk_ge, spelling);
            case '>':        return parse_relop(tk_gt, spelling);

            case 'var':   return parse_keyword(tk_var, spelling); 
            case 'while':   return parse_keyword(tk_while, spelling); 
            case 'if':   return parse_keyword(tk_if, spelling); 
            case 'else':   return parse_keyword(tk_else, spelling); 
            case 'let':   return parse_keyword(tk_let, spelling); 
