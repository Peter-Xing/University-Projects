...
if (alg == "WSARB")
{
	...
	a = stoi(aa); //number of bits
	b = stoi(bb);//value of time interval
	size = stoi(cc);//working set window size
	...
	std::vector<vector<int>> ref; //same size as frames
	std::vector<int> fi;//if need fifo, this records first in time
	std::vector<string> ws;//working set of size size
	...
	for (int i = 0; i < rw.size(); ++i)
	{
		...
		else // all frames filled
		{
			...
			{
				//find which element to replace 
				//first check each element in frames,
				//check how many of each, in ws
				//if not exist, is 0
			
				std::vector<int> Nws;//same size as frames
				for (int j = 0; j < frames.size(); ++j)
				{
					int nu=0;//number of each element
					...
					Nws.push_back(nu); //push index 
				}
				//loop Nws, find the min number's index
				//if there are mutiple min, use arb
				int mini = Nws[0];
				int posit;//index of min
				...
				//check if there are several same minimum value in num
				int c1=0;
				for (size_t i = 0; i < Nws.size(); ++i)
				{
					if (Nws[i] == mini)
				...
				int Index1;
				if (c1 > 1)//use arb to find the index
				{
					std::vector<int> num;//stores the transformed value of each element
					int position;
					for (int k = 0; k < ref.size(); ++k)
				...
				//check if the Index1's element already in Dir
				for (size_t x = 0; x < Dir.size(); ++x)
				{
					if (frames[Index1] == Dir[x])
					{
						dirty++;
						Dir.erase(Dir.begin()+x);
				...
				frames.erase(frames.begin()+Index1);
				fi.erase(fi.begin()+Index1);
				ref.erase(ref.begin()+Index1);
				frames.push_back(pi[i]);
				fi.push_back(timer);
				ref.push_back(tail1);
				MISS++;
