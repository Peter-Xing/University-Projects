...
int main(int argc, char const *argv[])
{
	...
	while(timer<Ftime)
	{
		...
		else//one customer is being processed in p vector
		{
			...
			else if (q1.empty()&&p[0].priority>3)
			{
				...
				else if (c.ctkt>1&&c.cr==95)//this quantum ends, need to go back to q2 to wait
				{
					...
					//after ageing, put it back to q2
					q2.push_back(p[0]);
					p.clear();
			...
			else if(p[0].priority<=3)//current processing item is from q1
			{
				//process p[0] one time
				Customer c = p[0];
				if (c.ctkt>1&&c.cr<c.quantum-5)// process 1 time
				{
					p[0].ctkt--;
					p[0].runningtime+=5;
					p[0].cr+=5;
		...
		if (DEBUG_PRINT_QUEUE)
    	{
        cout << "Time:" << timer-5 << endl;
        cout << "name "
             << "arrival_time "
             << "tickets_required "
             << "running "
        ...
        cout << "queue p" << endl;   
        cout << p[0].ID;
            cout << setiosflags(ios::right) << setw(10) << p[0].arTime << setw(10) << p[0].ctkt
                 << setw(10) << p[0].runningtime << setw(20) << p[0].priority << setw(10) << p[0].waitRuns<<" "<<p[0].processCount << endl; 
        cout << "queue 1" << endl;
        for (int i = 0; i < q1.size(); i++)
            ...
            cout << tmp.ID;
            cout << setiosflags(ios::right) << setw(10) << tmp.arTime << setw(10) << tmp.ctkt
                 << setw(10) << tmp.runningtime << setw(20) << tmp.priority << setw(10) <<tmp.waitRuns<<" "<<tmp.processCount << endl;
            q1.push(tmp);
        	}
        cout << "queue 2" << endl;
        for (int i = 0; i < q2.size(); i++)
        	{
            cout << q2[0].ID;
            cout << setiosflags(ios::right) << setw(10) << q2[i].arTime << setw(10) << q2[i].ctkt
                 << setw(10) << q2[i].runningtime << setw(20) << q2[i].priority << setw(10) << q2[i].waitRuns<<" "<<q2[i].processCount  << endl;
        	}
    	}
